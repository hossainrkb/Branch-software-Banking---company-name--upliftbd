...
function get_all_branch_product_list_by_branch_id($branch_id){
    $var = array();
    $results = mysql_query("SELECT * FROM tbl_branch_products WHERE bp_branch = '".$branch_id."' ORDER BY bp_id ASC ");
    if($results){
        while($row = mysql_fetch_array($results)){
            $var[] = $row;
        }
...
function get_branch_product_info_by_id($bp_id){
    $pro = array();
    $qu = mysql_query("SELECT * FROM  tbl_branch_products WHERE bp_id = '".$bp_id."' ");
...
function get_order_process_info_by_branch_id($branch_id){
    $var = array();
    $op_employee='0';
    $results = mysql_query("SELECT * FROM tbl_order_process WHERE op_branch_id = '".$branch_id."' AND op_employee='".$op_employee."' ORDER BY op_id ASC ");
    if($results){
        while($row = mysql_fetch_array($results)){
            $var[] = $row;
        }
...
function get_users_order_info_by_branch_nd_order_id($branch_id,$order_id){
    $results = mysql_query("SELECT * FROM tbl_users_order WHERE uo_branch_id='".$branch_id."' AND uo_order_id='".$order_id."'");
    $order = array();
    if($results){
        $order = mysql_fetch_array($results);
    }
    return $order;
}

function get_branch_employee_by_type($e_type,$e_branch_id){
    $var = array();
    $results = mysql_query("SELECT * FROM tbl_employee_info WHERE e_type='".$e_type."' AND e_branch='".$e_branch_id."'");
    if($results){
        while($row = mysql_fetch_array($results)){
            $var[] = $row;
        }
